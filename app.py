...
st.write("このアプリは LangChain を使い、OpenAI API にプロンプトを送って応答を表示します。\n\n注意: デプロイ時は Streamlit のシークレットに OPENAI_API_KEY を設定してください。")
...
def generate_answer(prompt: str, role: str) -> str:
    ...
    if role == "旅行ガイド":
        system_msg = "あなたは経験豊富な旅行ガイドです。旅行のおすすめ、旅程の作成、現地情報の注意点を分かりやすく説明してください。日本語で答えてください。"
    else:
        system_msg = "あなたはプロの栄養士です。バランスの良い食事のアドバイスや栄養計算、健康的な献立の提案を日本語で行ってください。"
