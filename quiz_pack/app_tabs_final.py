...
st.title("🇰🇷 韓国語文法 学習アプリ（文法解説＋発展＋練習）")
...
with tab1:
    ...
    if not text:
        ...
        st.markdown("---")
        st.markdown("#### 💡 発展解説（国立国語院・東京外国語大学 参考）")
        st.markdown(
            """
        国立国語院（한국어문법 통합자료）および
        東京外国語大学『朝鮮語文法モジュール』の情報をもとに、
        より深い用法・類義表現の違いをまとめています。
        """
        )
...
# ===========================
with tab2:
    st.subheader("🧩 練習モード選択")
    mode = st.radio("練習タイプを選んでください：", ["4択問題", "穴埋め問題", "作文問題"], horizontal=True)
    st.markdown("---")
    ...
    if not questions:
        ...
        # 穴埋め：学習目標の概念確認
            ...
            st.markdown("### ✏️ 学習目標の理解チェック（概念確認）")
            # 1〜2問だけ表示
            clozes = [
                {
